...
class ImageText(object):
    def __init__(self, filename_or_size, mode='RGBA', background=(0, 0, 0, 0),
                 encoding='utf8', maximum_font_size=12):
    ...
    def write_text(self, xy, text, font_filename, font_size=11,
        ...
        if font_size>self.maximum_font_size:
            font_size=self.maximum_font_size
        font = ImageFont.truetype(font_filename, font_size)
        if x == 'center':
            x = (self.size[0] - text_size[0]) / 2
        if y == 'center':
            y = (self.size[1] - text_size[1]) / 2
        self.draw.text((x, y), text, font=font, fill=color, anchor="mm")
        return text_size
